...
{
#ifdef RTW_GO_EXPERIMENT
	os_memcpy(ssid, &p2p->cfg->ssid_postfix[1], p2p->cfg->ssid_postfix_len-1);
...
#ifdef RTW_GO_EXPERIMENT
	os_memcpy(params->passphrase, "12121212", 8);
	return 0;
#endif
...
#ifdef CONFIG_WFD
	if (dev->info.wfd_enable) {
		res = os_snprintf(pos, end - pos,
				  "wfd_session_avail=%x\n"
				  "wfd_rtsp_ctrlport=%d\n"
				  "wfd_device=%d\n",
						  dev->info.session_avail,
						  dev->info.rtsp_ctrlport,
						  dev->info.wfd_device_type);
		if (res < 0 || res >= end - pos)
			return pos - buf;
		pos += res;
	}
