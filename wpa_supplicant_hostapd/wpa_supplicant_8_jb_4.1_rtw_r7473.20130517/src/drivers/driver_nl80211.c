			    ...
			    const u8 *frame, size_t len)
{
	...
	mgmt = (const struct ieee80211_mgmt *) frame;
#if (defined (CONFIG_AP) || defined (HOSTAPD) ) && defined (RTW_USE_LAGACY_STA_EVENT)
...
#ifdef RTW_USE_LAGACY_STA_EVENT
static void rtw_new_station_event(struct wpa_driver_nl80211_data *drv,
				      struct nlattr **tb)
{
	u8 *addr;
	struct ieee80211_mgmt *mgmt;
	int len = nla_len(tb[NL80211_ATTR_FRAME]);

	if (tb[NL80211_ATTR_FRAME]==NULL)
		return ;

	mgmt = (struct ieee80211_mgmt *) nla_data(tb[NL80211_ATTR_FRAME]);
	...
	if (is_ap_interface(drv->nlmode) && drv->device_ap_sme) {
		...
		ies = (u8 *) mgmt->u.assoc_req.variable;
		ies_len = len - 24 - sizeof(mgmt->u.assoc_req);
...
#ifdef RTW_USE_LAGACY_STA_EVENT
int check_assoc_frame(struct wpa_driver_nl80211_data *drv,
		       enum nl80211_commands cmd, struct nlattr *frame)
{
	const struct ieee80211_mgmt *mgmt;
	u16 fc, stype;
	...
	mgmt = (const struct ieee80211_mgmt *) (nla_data(frame));
	if ( nla_len(frame) < 24) {
		wpa_printf(MSG_DEBUG, "nl80211: Too short action frame");
		return FALSE;
	}
	...
	fc = le_to_host16(mgmt->frame_control);
	stype = WLAN_FC_GET_STYPE(fc);
	return  	((cmd == NL80211_CMD_FRAME) && (stype == WLAN_FC_STYPE_ASSOC_REQ));
}
#endif
...
static int process_bss_event(struct nl_msg *msg, void *arg)
{
	...
	case NL80211_CMD_FRAME_TX_STATUS:
#ifdef RTW_USE_LAGACY_STA_EVENT
		if (check_assoc_frame(bss->drv, gnlh->cmd, tb[NL80211_ATTR_FRAME]))
			rtw_new_station_event(bss->drv, tb);
		else
#endif
...
static int wiphy_info_handler(struct nl_msg *msg, void *arg)
{
	...
	if (tb[NL80211_ATTR_MAX_NUM_SCAN_SSIDS]) {
		...
		wpa_printf(MSG_INFO, "nl80211: NL80211_ATTR_MAX_NUM_SCAN_SSIDS:%d"
			, capa->max_scan_ssids);
	}
	...
	if (tb[NL80211_ATTR_MAX_NUM_SCHED_SCAN_SSIDS]) {
		...
		wpa_printf(MSG_INFO, "nl80211: NL80211_ATTR_MAX_NUM_SCHED_SCAN_SSIDS:%d"
			, capa->max_sched_scan_ssids);
	}
	...
	if (tb[NL80211_ATTR_MAX_MATCH_SETS]) {
		...
		wpa_printf(MSG_INFO, "nl80211: NL80211_ATTR_MAX_MATCH_SETS:%d"
			, capa->max_match_sets);
	}
...
#ifdef RTW_USE_LAGACY_STA_EVENT
	p2p_client_supported  = 1; p2p_go_supported = 1;
#endif
	if (tb[NL80211_ATTR_INTERFACE_COMBINATIONS]) {
		...
		nla_for_each_nested(nl_combi,
			...
			wpa_printf(MSG_INFO, "nl80211: get nl_combi");
...
broken_combination:
	...
	if (tb[NL80211_ATTR_OFFCHANNEL_TX_OK]) {
		...
		wpa_printf(MSG_INFO, "nl80211: NL80211_ATTR_OFFCHANNEL_TX_OK");
	}

	if (tb[NL80211_ATTR_ROAM_SUPPORT]) {
		wpa_printf(MSG_DEBUG, "nl80211: Using driver-based roaming");
		capa->flags |= WPA_DRIVER_FLAGS_BSS_SELECTION;
		wpa_printf(MSG_INFO, "nl80211: NL80211_ATTR_ROAM_SUPPORT");
	}
	...
	if (tb[NL80211_ATTR_SUPPORT_AP_UAPSD]) {
		...
		wpa_printf(MSG_INFO, "nl80211: NL80211_ATTR_SUPPORT_AP_UAPSD");
	}

	if (tb[NL80211_ATTR_MAX_REMAIN_ON_CHANNEL_DURATION]) {
		capa->max_remain_on_chan =
			nla_get_u32(tb[NL80211_ATTR_MAX_REMAIN_ON_CHANNEL_DURATION]);
		wpa_printf(MSG_INFO, "nl80211: NL80211_ATTR_MAX_REMAIN_ON_CHANNEL_DURATION:%u"
			, capa->max_remain_on_chan);
	}
	...
	if (tb[NL80211_ATTR_TDLS_SUPPORT]) {
		wpa_printf(MSG_INFO, "nl80211: TDLS supported");
		capa->flags |= WPA_DRIVER_FLAGS_TDLS_SUPPORT;

		if (tb[NL80211_ATTR_TDLS_EXTERNAL_SETUP]) {
			wpa_printf(MSG_INFO, "nl80211: TDLS external setup");
			capa->flags |=
	...
	if (tb[NL80211_ATTR_PROBE_RESP_OFFLOAD]) {
		...
		wpa_printf(MSG_INFO, "nl80211: Supports Probe Response "
			   "offload in AP mode");
...
#if defined (CONFIG_P2P) && defined (RTW_USE_LAGACY_STA_EVENT)
	nl80211_register_action_frame(bss, (u8 *)"\x7f\x50\x6f\x9a\x09", 5);
	nl80211_register_action_frame(bss, (u8 *)"\x04\x09\x50\x6f\x9a\x09", 6);
#endif
	if (nl80211_register_frame(bss, bss->nl_mgmt,
				   (WLAN_FC_TYPE_MGMT << 2) |
...
static int rtl871x_set_hidden_ssid(const char *iface, struct wpa_driver_ap_params *params)
{
	int ret;
	ieee_param ieee_param;
	size_t sz;

	wpa_printf(MSG_INFO, "%s ignore_broadcast_ssid:%d, %s,%d\n", __func__
		, params->hide_ssid
		, params->ssid
		, params->ssid_len
	);

	ieee_param.cmd = RTL871X_HOSTAPD_SET_HIDDEN_SSID;

	ieee_param.u.bcn_ie.reserved[0] = 0xea;
	ieee_param.u.bcn_ie.reserved[1] = (u8)params->hide_ssid;

	ieee_param.u.bcn_ie.buf[0] = WLAN_EID_SSID;
	ieee_param.u.bcn_ie.buf[1] = (u8)params->ssid_len;
	memcpy(ieee_param.u.bcn_ie.buf+2, params->ssid, params->ssid_len);

	sz = params->ssid_len + 2 + 12 + 2;// 12+2 = cmd+sta_addr+reserved, sizeof(ieee_param)=64, no packed

	ret = rtl871x_hostapd_ioctl(iface, &ieee_param, sz);
				     ...
				     struct wpa_driver_ap_params *params)
{
	...
	#ifdef RTL871X_HIDDEN_SSID_SUPPORT
	rtl871x_set_hidden_ssid(bss->ifname, params);
	#endif
...
static void nl80211_teardown_ap(struct i802_bss *bss)
{
	...
	if (drv->device_ap_sme) {
		wpa_driver_nl80211_probe_req_report(bss, 0);
#ifndef RTW_USE_LAGACY_STA_EVENT
		if (!drv->use_monitor)
#endif
