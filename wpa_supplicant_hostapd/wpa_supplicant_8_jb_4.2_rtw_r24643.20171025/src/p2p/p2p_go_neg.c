					    ...
					    struct p2p_device *peer)
{
	...
	int best_reg_class = 81, best_ch = 1;
	...
	if ((p2p->cfg->cfg_op_channel == 0) && 
	      p2p_best_channel_from_intersection( p2p->cfg, &p2p->channels,
					&best_ch, &best_reg_class)) {
		p2p->op_reg_class = p2p->cfg->op_reg_class = best_reg_class;
		p2p->op_channel = p2p->cfg->op_channel  = best_ch;
			    ...
			    const u8 *data, size_t len, int rx_freq)
{
	...
	if (dev && dev->flags & P2P_DEV_USER_REJECTED) {
		...
		if (go) {
			...
			if ((p2p->cfg->cfg_op_channel == 0) && 
				p2p_best_channel_from_intersection( p2p->cfg, 
								&intersection,
								&best_ch, 
								&best_reg_class)) {
				p2p->op_reg_class = p2p->cfg->op_reg_class = best_reg_class;
			     ...
			     const u8 *data, size_t len, int rx_freq)
{
	...
	if (go) {
		...
		if ((p2p->cfg->cfg_op_channel == 0) && 
			p2p_best_channel_from_intersection( p2p->cfg, &intersection,
							&best_ch, &best_reg_class)) {
			p2p->cfg->op_reg_class = best_reg_class;
			p2p->cfg->op_channel  = best_ch;
			p2p->cfg->cfg_op_channel = 1;
