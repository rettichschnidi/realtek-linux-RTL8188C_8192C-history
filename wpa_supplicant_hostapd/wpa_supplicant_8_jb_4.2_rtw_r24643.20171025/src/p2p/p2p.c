...
struct p2p_data * p2p_init(const struct p2p_config *cfg)
{
	...
	if (cfg->best_channel_path)
		p2p->cfg->best_channel_path = os_strdup(cfg->best_channel_path);
	if (cfg->pref_chan) {
...
void p2p_deinit(struct p2p_data *p2p)
{
	...
	os_free(p2p->cfg->best_channel_path);
	os_free(p2p->groups);
...
u8* p2p_get_inv_go_dev_addr(struct p2p_data *p2p)
{
	return p2p->inv_go_dev_addr;
}
