...
static void hostapd_reload_bss(struct hostapd_data *hapd)
{
	...
	//RTK patched: for filtering macaddr
		...
		hapd->conf->num_deny_mac > 0)
	{
		...
		char buf[256] = {0x00};
		int num_entry = 0, macaddr_acl = 0, i = 0;
		struct mac_acl_entry *mac_entry = NULL;

		if(hapd->conf->macaddr_acl == 0)
			macaddr_acl = 1;
		else if(hapd->conf->macaddr_acl == 1)
			macaddr_acl = 2;
		else
			macaddr_acl = 0;
		os_snprintf(buf, sizeof(buf), "HOSTAPD_SET_MACADDR_ACL = %d", macaddr_acl);

		hostapd_drv_driver_cmd(hapd, buf, buf, sizeof(buf));

		os_memset( buf, 0x00, 256 );
		//Baddr is used for clearing
		os_snprintf(buf, sizeof(buf), "HOSTAPD_ACL_REMOVE_STA = ff:ff:ff:ff:ff:ff");
		hostapd_drv_driver_cmd(hapd, buf, buf, sizeof(buf));
		
		if(hapd->conf->macaddr_acl  == 0 &&
			hapd->conf->num_deny_mac > 0)
		{
			mac_entry = (struct mac_acl_entry *)hapd->conf->deny_mac;
			num_entry = hapd->conf->num_deny_mac;
		}
		else if (hapd->conf->macaddr_acl  == 1 &&
			hapd->conf->num_accept_mac > 0)
		{
			mac_entry = (struct mac_acl_entry *)hapd->conf->accept_mac;
		...
		{
			os_memset( buf, 0x00, 256 );
			os_snprintf(buf, sizeof(buf), "HOSTAPD_ACL_ADD_STA = "MACSTR"",
				MAC2STR((mac_entry+i)->addr) );
			hostapd_drv_driver_cmd(hapd, buf, buf, sizeof(buf));
		}
