				     ...
				     const u8 *ssid, size_t ssid_len)
{
	...
	if (p2p == NULL) {
		...
		os_memcpy(go_dev_addr, p2p_get_inv_go_dev_addr(wpa_s->global->p2p), ETH_ALEN);
		return 1;
		//return 0;
	}
				     ...
				     int op_freq)
{
	...
	if (status == P2P_SC_SUCCESS) {
		...
		if (s) {
			...
			/** 
			 * RTK patched: for some platforms, the op channel it said in p2p invitation req/rsp frame
			 * may differ from the real ch it located.
			 */
#ifdef CONFIG_P2P_PERSISTENT_CHECK_SOCIAL_CH
			wpa_s->p2p_persistent_check_social_ch = 1;	// To figure out we are doing persistent reconnection
...
int wpas_p2p_init(struct wpa_global *global, struct wpa_supplicant *wpa_s)
{
	...
	p2p.best_channel_path = wpa_s->conf->best_channel_path;
	if (wpa_s->wps) {
