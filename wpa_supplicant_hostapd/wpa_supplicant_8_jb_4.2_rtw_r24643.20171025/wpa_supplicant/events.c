...
#ifdef CONFIG_CONNECT_CH_CONFLICT_DETECT
#include "p2p/p2p_i.h"
#endif
...
#ifdef CONFIG_CONNECT_CH_CONFLICT_DETECT
	struct wpa_supplicant *wpa_s_temp=NULL;
	Boolean flag_ch_conflict = FALSE;
#endif
...
#ifdef CONFIG_CONNECT_CH_CONFLICT_DETECT
		for (wpa_s_temp = wpa_s->global->ifaces; wpa_s_temp; wpa_s_temp = wpa_s_temp->next) {
			if (!((wpa_s->global->p2p->cfg) && (wpa_s->global->p2p->cfg) && (wpa_s->global->p2p->cfg->cb_ctx)))
				break;
			if (wpa_s != (struct wpa_supplicant *)wpa_s->global->p2p->cfg->cb_ctx)
				break;
			wpa_msg(wpa_s, MSG_INFO, "wpa_s_temp: ifname=%s, wpa_state=%d,  assoc_freq=%d\n ",
				wpa_s_temp->ifname, wpa_s_temp->wpa_state, wpa_s_temp->assoc_freq);
			if ((wpa_s_temp != wpa_s) && 
			     (wpa_s_temp->wpa_state ==WPA_COMPLETED) && 
			     (wpa_s_temp->assoc_freq !=0)) {	
				if (wpa_s_temp->assoc_freq != (unsigned int)selected->freq) {
					wpa_msg(wpa_s, MSG_INFO, "Warming: channel conflict!! ifname=%s, freq=%d && ifname=%s, freq=%d\n",
						wpa_s_temp->ifname, wpa_s_temp->assoc_freq, wpa_s->ifname, selected->freq);
					flag_ch_conflict = TRUE;
					break;
				}
			}
		}
...
//					" id=%d", ssid->id);
			wpa_msg(wpa_s, MSG_INFO, P2P_EVENT_GO_NEG_FAILURE 
				"status=%d", P2P_SC_FAIL_NO_COMMON_CHANNELS);
			os_memset(wpa_s->pending_bssid, 0, ETH_ALEN);
			return   -1;
		}
#if 0
		if (wpa_s->go_params) {
			wpa_msg(wpa_s, MSG_DEBUG, "\n!!!	wpa_s->go_params.freq = %d	,   selected->freq=%d   !!!\n",
					wpa_s->go_params->freq, selected->freq);
			if  (wpa_s->go_params->freq != selected->freq)	{
				wpa_supplicant_disable_network(wpa_s, ssid);
//				wpa_msg(wpa_s, MSG_INFO, WPA_EVENT_FREQ_CONFLICT
//						" id=%d", ssid->id);

				wpa_msg(wpa_s, MSG_INFO, P2P_EVENT_GO_NEG_FAILURE "status=%d", P2P_SC_FAIL_NO_COMMON_CHANNELS);
				os_memset(wpa_s->pending_bssid, 0, ETH_ALEN);
				return   -1;
			}
		}
