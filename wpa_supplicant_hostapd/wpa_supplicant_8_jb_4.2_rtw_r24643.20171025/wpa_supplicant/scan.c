	...
	struct wpa_supplicant *wpa_s, struct wpa_driver_scan_params *params)
{
	...
	if (params->freqs == NULL && wpa_s->after_wps && wpa_s->wps_freq) {
		...
		if( wpa_s->p2p_persistent_check_social_ch )
		{
			wpa_dbg(wpa_s, MSG_DEBUG, "WPS: Additionally check social channels");
			params->freqs = os_zalloc(5 * sizeof(int));
			if (params->freqs)
			{
				params->freqs[0] = 2412;
				params->freqs[1] = 2437;
				params->freqs[2] = 2462;
				if( wpa_s->wps_freq != 2412 && wpa_s->wps_freq != 2437 && wpa_s->wps_freq != 2462 )
					params->freqs[3] = wpa_s->wps_freq;
			}
		}
		else
		{
