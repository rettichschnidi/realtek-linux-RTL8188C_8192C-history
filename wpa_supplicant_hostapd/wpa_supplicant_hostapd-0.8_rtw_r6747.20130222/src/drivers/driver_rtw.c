...
static u8 * get_ie(const u8 *ies, size_t ies_len, u8 id)
{
	u8 *end, *pos;
			  ...
			  int beacon_int)
{
	...
	if(hapd->conf->ignore_broadcast_ssid == 2)
	{
		...
		os_memcpy(ssid_ie+2, hapd->conf->ssid.ssid, ssid_len);
	}
	...
	{
		u8 *ssid_ie_next = (u8*)(head+24+12+2);
		size_t head_remain_len = head_len-24-12-2;

		os_memcpy(pparam->u.bcn_ie.buf, (head+24), 12); 
		
		pparam->u.bcn_ie.buf[12] = WLAN_EID_SSID;
		pparam->u.bcn_ie.buf[13] = expend_len;
		os_memcpy(pparam->u.bcn_ie.buf+12+2, hapd->conf->ssid.ssid, expend_len);
		
		os_memcpy(pparam->u.bcn_ie.buf+12+2+expend_len, ssid_ie_next, head_remain_len);// 24=beacon header len.	
		os_memcpy(&pparam->u.bcn_ie.buf[head_len-24+expend_len], tail, tail_len);
	}
	else
#endif //RTL871X_HIDDEN_SSID_SUPPORT
	{
